...
static void tegra_pcie_hotplug_init(void)
{
	...
	for (nr = 0; nr < tegra_pcie_hw.nr_controllers; nr++) {
		...
		if (ret > 0)
			pci_create_root_bus(NULL, nr, &tegra_pcie_ops, sys, NULL);
	}
...
static void __devinit work_hotplug_handler(struct work_struct *work)
{
...
static int __devinit tegra_pcie_resume(struct device *dev)
{
	int ret = 0;
